...
public enum DocumentType {
    ...
    public static DocumentType detectFromContent(String text) {
        ...
        if (containsAny(lowerText, "lasku", "laskun", "laskutus", "maksettava", "eräpäivä", 
                       "yhteensä", "alv", "arvonlisävero", "summa", "hinta", "maksu")) {
            return INVOICE;
        }
        
        // Receipt detection patterns
        if (containsAny(lowerText, "kuitti", "ostos", "kassalaite", "kassa", "myymälä", 
                       "kauppa", "osto", "maksutapa")) {
            return RECEIPT;
        }
        
        // Contract detection patterns
        if (containsAny(lowerText, "sopimus", "sopimusehto", "osapuoli", "allekirjoitus", 
                       "sitoumus", "velvoite")) {
            return CONTRACT;
        }
        
        // Certificate detection patterns
        if (containsAny(lowerText, "todistus", "sertifikaatti", "diplomí", "pätevyys", 
                       "koulutus", "suoritus")) {
            return CERTIFICATE;
        }
        
        // Report detection patterns
        if (containsAny(lowerText, "raportti", "selvitys", "analyysi", "tutkimus", 
                       "yhteenveto", "tilasto")) {
            return REPORT;
        }
        
        // Form detection patterns
        if (containsAny(lowerText, "lomake", "hakemus", "ilmoitus", "rekisteröinti", 
                       "täytä", "allekirjoita")) {
            return FORM;
        }
        
        // Letter detection patterns
        if (containsAny(lowerText, "kirje", "viesti", "tervehdys", "kunnioittaen", 
                       "ystävällisin", "yhteistyöterveisin")) {
            return LETTER;
        }
        
        // Manual detection patterns
        if (containsAny(lowerText, "käyttöohje", "ohje", "opas", "manual", "instructions", 
                       "vaihe", "askel")) {
            return MANUAL;
        }
        
        // Specification detection patterns
        if (containsAny(lowerText, "erittely", "listaus", "luettelo", "yksityiskohta", 
