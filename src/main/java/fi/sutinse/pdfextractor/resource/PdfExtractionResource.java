...
public class PdfExtractionResource {
    ...
    public Response extractText(@RestForm("file") FileUpload file) {
        ...
        try {
            ...
            if (response.success()) {
                LOGGER.info("PDF extraction successful for file: {}, method: {}, type: {}", 
                           filename, response.methodUsed(), response.documentType());
                return Response.ok(response).build();
            } else {
                LOGGER.error("PDF extraction failed for file: {}, error: {}", 
    ...
    public Response health() {
        return Response.ok()
               .entity(new HealthResponse("PDF Extractor Service", "UP"))
               .build();
    }
